  ...
  return (
    <div className="container max-w-4xl mx-auto px-4 py-8">
      <header className="mb-8">
        ...
        <p className="text-muted-foreground">Hantera dina konversationer kring fordon</p>
      </header>
      ...
      {conversations && conversations.length > 0 ? (
        <div className="space-y-3">
          {conversations.map((convo: Conversation) => {
            ...
            const partnerName = convo.participantNames[partnerId || ''] || convo.participantEmails?.[partnerId || ''] || 'Användare';
            const isUnread = convo.unreadBy?.includes(user.uid);
            const lastDate = convo.lastMessageAt?.toDate ? convo.lastMessageAt.toDate() : new Date();
                  ...
                  <CardContent className="p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-sm truncate ${isUnread ? 'text-foreground font-semibold' : 'text-muted-foreground'}`}>
                        {convo.lastMessageSenderId === user.uid ? 'Du: ' : ''}{convo.lastMessage}
                      </p>
                    </div>
                  </CardContent>
                </Card>
