...
function PartnerInfo({ userId, fallbackName, showName = true }: { userId: string, fallbackName: string, showName?: boolean }) {
  ...
  const banRef = useMemoFirebase(() => {
    if (!db || !userId) return null;
    return doc(db, 'artifacts', appId, 'public', 'data', 'bannedUsers', userId);
  }, [db, userId, appId]);
  ...
  if (!showName) {
    return (
      <div className="relative shrink-0">
        <Avatar className={`h-10 w-10 ${isWorkshop ? 'rounded-lg' : 'rounded-full'} border-2 ${isBanned ? 'border-red-500' : 'border-transparent'} shadow-sm`}>
  ...
  return (
    <div className="flex items-center gap-3 min-w-0">
      <div className="relative shrink-0">
        <Avatar className={`h-8 w-8 ${isWorkshop ? 'rounded-lg' : 'rounded-full'} border-2 ${isBanned ? 'border-red-500' : 'border-transparent'} shadow-sm`}>
          <AvatarImage src={profile?.photoUrl} className="object-cover" />
