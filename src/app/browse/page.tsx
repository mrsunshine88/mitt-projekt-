...
function VehicleOwnerAvatar({ ownerId }: { ownerId?: string }) {
  const db = useFirestore();
  const appId = firebaseConfig.projectId;
  const profileRef = useMemoFirebase(() => {
    if (!db || !ownerId) return null;
    return doc(db, 'artifacts', appId, 'public', 'data', 'public_profiles', ownerId);
  }, [db, ownerId, appId]);
  const { data: profile } = useDoc<any>(profileRef);
  
  const banRef = useMemoFirebase(() => {
    if (!db || !ownerId) return null;
    return doc(db, 'artifacts', appId, 'public', 'data', 'bannedUsers', ownerId);
  }, [db, ownerId, appId]);
  const { data: banDoc } = useDoc<any>(banRef);
  const isBanned = !!banDoc;
  ...
  return (
    <div className="flex items-center gap-2 mt-2">
      <Avatar className={`w-5 h-5 ${isWorkshop ? 'rounded-md' : 'rounded-full'} border-[1px] ${isBanned ? 'border-red-500' : 'border-transparent'}`}>
        <AvatarImage src={profile.photoUrl || undefined} className="object-cover" />
        <AvatarFallback className="bg-primary/20 text-primary text-[8px] font-bold">{profile.name?.[0]}</AvatarFallback>
      </Avatar>
      <span className={`text-[10px] uppercase font-bold truncate max-w-[120px] ${isBanned ? 'text-red-500/70 line-through' : 'text-muted-foreground'}`}>{profile.name}</span>
    </div>
  );
}
        ...
        <CardContent className="p-5 space-y-4 flex-1 flex flex-col">
          <div className="flex justify-between items-start">
            <div>
              ...
              <VehicleOwnerAvatar ownerId={vehicle.ownerId || undefined} />
              <div className="flex items-center gap-3 mt-2 text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
