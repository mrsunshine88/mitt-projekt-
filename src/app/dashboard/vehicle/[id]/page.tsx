...
export default function PrivateVehicleProfile({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeleteFromGarage = async () => {
    ...
    try {
      ...
      await batch.commit();
      toast({ title: "Borttagen från garaget och bilderna rensade. Historiken är sparad." });
      router.push('/dashboard');
    } catch (e: any) { toast({ variant: "destructive", title: "Fel", description: e.message }); } 
  ...
  const handleAdminHardDelete = async () => {
    if (!db || !(isAdminContext && isHuvudAdmin) || hardDeleteConfirm !== 'RADERA') return;
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-8 pt-4">
                ...
                <HistoryList 
                  ...
                  isHuvudAdminMode={isAdminContext && isHuvudAdmin}
                />
              </div>
            </div>
          </div>
