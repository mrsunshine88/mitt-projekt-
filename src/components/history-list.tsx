...
export function HistoryList({ logs, showPrivateData = false, onEdit, onDelete, onApprove, onReject, isHuvudAdminMode = false }: any) {
  const { user } = useUser();
  const db = useFirestore();
  const appId = firebaseConfig.projectId;
  ...
  return (
    <div className="space-y-6">
      {displayLogs.map((log: VehicleLog) => {
        ...
        const canSeeSensitive = isOwnerAtTime || isHuvudAdminMode || isCreator;
        ...
        const canEdit = onEdit && (isHuvudAdminMode || (isCreator && !isOwnershipTransfer));
